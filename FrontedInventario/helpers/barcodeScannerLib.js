...
const LONGITUD_MIN_CODIGO_BARRAS = 3; // Mínimo para códigos de barras (reducido para soportar códigos cortos)
...
function _leerCodigoBarras(event) {
  ...
  const tiempoTranscurrido =
    _tiempoUltimaTecla > 0 ? ahora - _tiempoUltimaTecla : 999;

  // Si ha pasado demasiado tiempo, reiniciar (lectura manual, no escáner)
  if (
    tiempoTranscurrido > TIEMPO_MAXIMO_ENTRE_TECLAS &&
    _codigoBarras.length > 0
  ) {
    _resetearEscaner();
  }
  ...
  if (event.key.length === 1 || event.key === "Enter") {
    ...
    const tieneTeclasConsistentes =
      _velocidadesTeclas.length >= MIN_TECLAS_RAPIDAS_CONSECUTIVAS &&
      _velocidadesTeclas
        .slice(-MIN_TECLAS_RAPIDAS_CONSECUTIVAS)
        .every((v) => v < TIEMPO_MAXIMO_ENTRE_TECLAS);
    ...
    if (
      tieneTeclasConsistentes &&
      esPatronCodigoBarras &&
      _codigoBarras.length >= 2
    ) {
    ...
    if (_codigoBarras.length >= LONGITUD_MIN_CODIGO_BARRAS) {
      ...
      _timeoutLimpieza = setTimeout(() => {
        if (
          _codigoBarras.length >= LONGITUD_MIN_CODIGO_BARRAS &&
          esPatronCodigoBarras
        ) {
          _procesarCodigoBarras();
        }
...
function _limpiarCampoTemporal() {
  if (
    _campoLectura &&
    (_campoLectura.tagName === "INPUT" || _campoLectura.tagName === "TEXTAREA")
  ) {
    ...
    for (let i = _codigoBarras.length; i > 0; i--) {
      ...
      if (valorActual.endsWith(fragmento)) {
        _campoLectura.value = valorActual.substring(
          0,
          valorActual.length - fragmento.length,
        );
        break;
      }
    }
