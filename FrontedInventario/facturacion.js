...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const enviarPorCorreoBtn = document.getElementById('enviarPorCorreoBtn');
    if (enviarPorCorreoBtn) {
        enviarPorCorreoBtn.addEventListener('click', () => {
...
function enviarFacturaPorCorreo() {
    ...
    const correoCliente = document.getElementById('emailCliente').value.trim();
    const productos = obtenerProductosSeleccionados();

    if (nombreCliente && cedulaNit && correoCliente && productos.length > 0) {
        // Generar el PDF usando jsPDF
        const doc = new jspdf.jsPDF();
        doc.text("Factura de Venta", 10, 10);
        // Aquí agrega el contenido de la factura al PDF...
        const pdfBlob = doc.output('blob');

        const formData = new FormData();
        formData.append('pdf', pdfBlob, 'factura.pdf');
        formData.append('to', correoCliente); // Usar el correo ingresado
        formData.append('subject', 'Tu factura');
        formData.append('text', 'Adjunto encontrarás tu factura.');
